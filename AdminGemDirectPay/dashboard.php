				...
				<div class='row'>
					<div class="col-xs-12" style="margin-right: 25px">
						<div class="row float-right">
							<button type="button" onclick="dateFilterButton(this)" id="1" class="btn btn-dashboard waves-effect waves-light dashboardBtn">All Time</button>
							<button type="button" onclick="dateFilterButton(this)" id="2" class="btn btn-dashboard waves-effect waves-light dashboardBtn">Today</button>
							<button type="button" onclick="dateFilterButton(this)" id="3" class="btn btn-dashboard waves-effect waves-light">Yesterday</button>
							<button type="button" onclick="dateFilterButton(    this)" id="4" class="btn btn-dashboard waves-effect waves-light">Last 7 Days</button>
							<button type="button" onclick="dateFilterButton(this)" id="5" class="btn btn-dashboard waves-effect waves-light">Last 30 Days</button>
...
<script>
	...
	var today;
	var endToday;
	var yesterday = moment(d).subtract(1, 'days').format('DD/MM/YYYY 00:00:00');
	var endYesterday = moment(d).subtract(1, 'days').format('DD/MM/YYYY 23:59:59');
	var testArr = [];

	$(document).ready(function() {


		today = moment(d).format('DD/MM/YYYY 00:00:00');
		endToday = moment(d).format('DD/MM/YYYY 23:59:59');
		...
		$("#coinTransactionDate").change(function() {
			if ($("#coinTransactionDate").val() == 'today') {
				...
				$("#toDate").val('');
				$('#dateRangeDisplay1').hide();
				today = moment(d).subtract(1, 'days').format('DD/MM/YYYY 00:00:00');
				endToday = moment(d).subtract(1, 'days').format('DD/MM/YYYY 23:59:59');
				loadCoinTransaction(today, endToday);
	...
	function dateFilterButton(elem) {
		if (elem.id == 1) {
			...
			today = moment(d).subtract(6, 'days').format('DD/MM/YYYY 00:00:00');
			endToday = moment().format('DD/MM/YYYY 23:59:59');
			dataValue = (today + " - " + endToday);
			runGraphData(today, endToday)

		} else if (elem.id == 5) {
			today = moment().subtract(29, 'days').format('DD/MM/YYYY 00:00:00');
			endToday = moment().format('DD/MM/YYYY 23:59:59');
			dataValue = (today + " - " + endToday);
			runGraphData(today, endToday)
		}
	...
	function loadCoinTransaction(today, endToday) {
		if ($("#fromDate").val()) {
			...
			fromDateTime = dateToTimestamp(today);
		}
		if ($("#toDate").val()) {
			toDateTime = dateToTimestamp($("#toDate").val() + " 23:59:59");
		} else {
			toDateTime = dateToTimestamp(endToday);
		}
	...
	function generatePiechart(piechartID, piechartdata, value, category) {
		...
		if (piechartID == 'piechart1' || piechartID == 'piechart2' || piechartID == 'piechart4') {
