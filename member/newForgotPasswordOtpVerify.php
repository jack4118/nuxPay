...
// console.log(countryCode);
	...
	$(document).ready(function(){
		...
		$('.digit-group').find('input').each(function() {
				...
				if (e.keyCode === 8 || e.keyCode === 37) {
					...
					if (!next.length) {
						...
						if(verify_code.length==5) {
							$(this).blur();
							submitOTPCode(countryCode, mobileNumber, email, accountType, verify_code);
						}
					...
					if(str.length==5) {
						...
						$(this).blur();
						//submitOTPCode(countryCode, mobileNumber, email, accountType, str);
					}
				}
			});
			...
			$('#signUpBtn').click(function() {
				submitOTPCode(countryCode, mobileNumber, email, accountType, otpCode);
		...
		$('#resendBtn').click(function() {
			...
			fCallback = resetMerchantPasswordVerifyCodeResult;
		...
		if(accountType=="email") {
			...
			$('#otpReceiver').text((countryCode + mobileNumber.replace(/^0+/, '')));
		}


		if( accountType=='' || (accountType=='email' && email=='') || (accountType=='mobile' && mobileNumber=='') ) {
	...
	function submitOTPCode(countryCode, mobileNumber, email, accountType, verify_code) {
		...
		if(verify_code.length==5) {
			...
			var url = '<?php echo $sys['domain'] ?>/scripts/reqLogin.php';
			var formData = {
				command: 'resetMerchantPasswordValidate',
				mobile: mobile,
				email: email,
				mode: accountType,
			...
			.done(function(data) {
				...
				if (obj.code == 1) {
					...
					if(accountType=='email') {
						utmTracking('', email, '', 'Forgot Password OTP Verify (' + obj.message_d + ')');
					} else {
						utmTracking('', mobile, '', 'Forgot Password OTP Verify (' + obj.message_d + ')');
					}
					

					var request_id = obj.data.request_id;
					var validate_id = obj.data.validate_id;

					$.redirect('newResetPassword.php', {'accountType': accountType, 'email': email, 'countryCode': countryCode, 'mobileNumber': mobileNumber, 'request_id': request_id, 'validate_id': validate_id, 'verify_code': verify_code});
					...
					if(accountType=='email') {
						utmTracking('', email, '', 'Forgot Password OTP Verify (' + error_message + ')');
					} else {
						utmTracking('', mobile, '', 'Forgot Password OTP Verify (' + error_message + ')');
					}
	...
	function resetMerchantPasswordVerifyCodeResult(data, message) {
		
		if(accountType=="email") {
			utmTracking('', email, '', 'Forgot Password Resend OTP Success');
		} else {
			utmTracking('', (countryCode + mobileNumber.replace(/^0+/, '')), '', 'Forgot Password Resend OTP Success');
		}
