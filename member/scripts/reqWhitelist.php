<?php
    ...
    **/
    ...
    switch($command) {
		...
		case "requestDisable2FASMSOTP":
			$params = array(
				'user_id' => $_SESSION["business"]["uuid"],
			);

			$result = $post->curl_post_whitelist("requestDisable2FASMSOTP", $params);

			echo $result;
        break;
			
		case "verifyDisable2FASMSOTP":
			$params = array(
				'user_id' => $_SESSION["business"]["uuid"],
				'otp_code' => $_POST['otp_code'],
			);

			$result = $post->curl_post_whitelist("verifyDisable2FASMSOTP", $params);
		...
		case "getApiList":
			...
			$result = $post->curl_post_whitelist("getApiList", $params);
		...
		case "getApiDetail":
			...
			$result = $post->curl_post_whitelist("getApiDetail", $params);
		...
		case "generateApiKey":
			...
			$result = $post->curl_post_whitelist("generateApiKey", $params);
		...
		case "revokeApiKey":
			...
			$result = $post->curl_post_whitelist("revokeApiKey", $params);
