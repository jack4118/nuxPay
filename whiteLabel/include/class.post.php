...
class post{
  ...
  public function curlUtm($command, $params = array(), $site="Admin", $language="english", $source="nuxpay", $sourceVersion="", $userAgent="")
  {
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/x-msgpack','user-agent: '.$userAgent, 'user-ip: '.$user_ip,'source: '.$source,'access-token: '.$_SESSION["access_token"],'business-id: '.$_SESSION["business"]["uuid"]));
...
//          curl_setopt($ch, CURLOPT_TIMEOUT, 60); //timeout in seconds
  ...
  public function curl_standard($command, $params = array(), $site="Admin", $language="english", $source="business", $sourceVersion="", $userAgent="")
  {
    ...
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/x-msgpack','source: '.$source,'access-token: '.$_SESSION["access_token"],'business-id: '.$_SESSION["business"]["uuid"]));
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
//        curl_setopt($ch, CURLOPT_TIMEOUT, 60); //timeout in seconds
    ...
    if($msgReturn['code'] == 3) {
      session_destroy();
            ...
            function curl_post($command, $params = array(), $site="Member", $language="english", $source="nuxpay", $sourceVersion="", $userAgent="") {
          //   include(__DIR__.'/config.php');
          //   include(__DIR__.'/class.msgpack.php');
    ...
    // if(!in_array($command, $filteredCommand)) $params["business_email"] = $_SESSION["business_email"];
              ...
              curl_setopt($ch, CURLOPT_URL, $webserviceURL);
              curl_setopt($ch, CURLOPT_POST, true);
              curl_setopt($ch, CURLOPT_HTTPHEADER, array('user-agent: '.$userAgent, 'user-ip: '.$user_ip, 'source: '.$source,'access-token: '.$_SESSION["access_token"],'business-id: '.$_SESSION["business"]["uuid"]));
              curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
              curl_setopt($ch, CURLOPT_POSTFIELDS, $params);
            ...
            function curl_post_multipart($command, $params = array()) {
              include(dirname(__FILE__)."/../include/config.php");
    ...
    // curl_setopt($ch, CURLOPT_HTTPHEADER, array('X-Xun-Token: '.$_SESSION["access_token"], 'Content-Type: multipart/form-data; boundary=' . $boundary));
            ...
            function curl_post_pass($command, $params = array()) {
              include(dirname(__FILE__)."/../include/config.php");
    ...
    // if(!in_array($command, $filteredCommand)) $params["business_email"] = $_SESSION["business_email"];
            ...
            function curl_get($command, $params = array()) {
              include(dirname(__FILE__)."/../include/config.php");
