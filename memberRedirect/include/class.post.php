...
//        curl_setopt($ch, CURLOPT_TIMEOUT, 60); //timeout in seconds
    ...
    // if(!in_array($command, $filteredCommand)) $params["business_email"] = $_SESSION["business_email"];
            ...
            function curl_post_multipart($command, $params = array(), $source = "NuxPay") {
              include(dirname(__FILE__)."/../include/config.php");
    ...
    // curl_setopt($ch, CURLOPT_HTTPHEADER, array('X-Xun-Token: '.$_SESSION["access_token"], 'Content-Type: multipart/form-data; boundary=' . $boundary));
            ...
            function curl_post_pass($command, $params = array(), $source = "NuxPay") {
              include(dirname(__FILE__)."/../include/config.php");
    ...
    // if(!in_array($command, $filteredCommand)) $params["business_email"] = $_SESSION["business_email"];
            ...
            function curl_get($command, $params = array(), $source = "NuxPay") {
              include(dirname(__FILE__)."/../include/config.php");
    ...
    // if(!in_array($command, $filteredCommand)) $params["business_email"] = $_SESSION["business_email"];
			...
			function curl_post_whitelist($command, $params = array(), $site="Member", $language="english", $source="NuxPay", $sourceVersion="", $userAgent="") {
              include(dirname(__FILE__)."/../include/config.php");

              $filteredCommand = $this->post();

              $webserviceURL = $sys['whitelistWebserviceURL'];
