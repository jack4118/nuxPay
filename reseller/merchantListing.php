...
<?php include("head.php"); ?>
            ...
            <div class="content">
                <div class="container">
                                        ...
                                        <div class="panel-body">
                                            ...
                                            <!-- Create On -->
                                                <div class="col-md-4 form-group" style="height: 62px;">
                                                    <label class="control-label">Created On Range</label>
                                                ...
                                                <!-- Last Login -->
                                                <div class="col-md-4 form-group" style="height: 62px;">
                                                    <label class="control-label">Last Login Range</label>
                                                    <div class="input-group input-daterange" id="m_datepicker_5">
                                                        <input id="fromDate1" type="text" class="form-control input-daterange-from" placeholder="From" dataName="startDate1" dataType="dateRange" autocomplete="off" name="start1" />
                                                        <span class="input-group-addon">
                                                            To
                                                        </span>
                                                        <input id="toDate1" type="text" class="form-control input-daterange-to" placeholder="To" dataName="endDate1" dataType="dateRange" autocomplete="off" name="end1" />
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-md-4 form-group" id="displayDiv">
                                                <label class="control-label" id="displayLabel">Display Type</label>
                                                <select id="displayType" class="form-control" dataName="displayType" dataType="text">
                                                    <option value="totalMerchant">Total Merchant</option>
                                                    <option value="direct"<?php if (isset($_GET['resellerUsername']) && $_GET['resellerUsername'] == 'direct') { echo ' selected'; } ?>>Direct</option>
                                                    <option value="indirect"<?php if (isset($_GET['resellerUsername']) && $_GET['resellerUsername'] == 'indirect') { echo ' selected'; } ?>>Indirect</option>
                                                </select>
                                                </div>
                    ...
                    <div class="row" style="margin-top: 20px;">
                        <div class="col-md-12">
                            <a href="createUser.php" class="btn btn-primary">Create Merchant</a>
                        </div>
   ...
   <script>
    ...
    var thArray  = Array('Merchant ID',
                         ...
                         'Last Login',
                         'Created On',
                         'Total Transaction (External Transfer)',
                         'Total Transaction (Swap)',
                         'Total Transaction (Redeem Code)',
                         'Total Transaction (Send Fund)',
                         'Total Transaction (PG Received Fund)',
                         'Total Transaction (API Intergration)',
                         'Total Transaction (Withdrawal)',
                         'Total Transaction (BC Fund In)',
                         'Total Transaction Amount (USD) (Fund In)',
                         'Total Commission Contributed Amount (USD) (Fund In)'
                        );
    ...
    $(document).ready(function() {
        ...
        $('#fromDate1').daterangepicker({
            ...
            $("#fromDate1").val(start1.format('DD/MM/YYYY'));
            $("#toDate1").val(end1.format('DD/MM/YYYY'));
        });
        $('#toDate1').click(function() {
            $('#fromDate1').trigger('click');
        });
        $('input[name="start"]').on('apply.daterangepicker1', function(ev, picker) {
            $('#fromDate1').val(picker.startDate1.format('DD/MM/YYYY'));
            $('#toDate1').val(picker.endDate1.format('DD/MM/YYYY'));
        });
    ...
    function pagingCallBack(pageNumber, fCallback){
        ...
        // Create On
        if ($("#fromDate").val()) {
            fromCreateDate = $("#fromDate").val() + " 00:00:00";
            fromCreateDate = fromCreateDate.replaceAll('/', '-');
        } else {
            fromCreateDate = "";
        }
        if ($("#toDate").val()) {
            toCreateDate = $("#toDate").val() + " 23:59:59";
            toCreateDate = toCreateDate.replaceAll('/', '-');
        } else {
            toCreateDate = "";
        }
        ...
        // Last Login
        if ($("#fromDate1").val()) {
            fromLoginDate = $("#fromDate1").val() + " 00:00:00";
            fromLoginDate = fromLoginDate.replaceAll('/', '-');
        } else {
            fromLoginDate = "";
        }
        if ($("#toDate1").val()) {
            toLoginDate = $("#toDate1").val() + " 23:59:59";
            toLoginDate = toLoginDate.replaceAll('/', '-');
        } else {
            toLoginDate = "";
        }
            ...
            if (merchantListing) {
                ...
                $.each(merchantListing, function(k,v){
                    ...
                    var rebuildData ={
                        ...
                        resellerUsername : v['reseller'],
                        distributorUsername : v['distributor'],
                        lastLogin : v["last_login"],
                        date : v['business_created_at'],
                        totalExternalTransfer : v['total_external_transfer'],
                        totalSwap : v['total_swap'],
                        totalRedeem : v['total_redeem'],
                        totalSendFund : v['total_send_fund'],
                        totalReceived : v['total_received'],
                        totalApiIntergration : v['total_api_intergration'],
                        totalWithdrawal : v['total_withdrawal'],
                        totalFundIn : v['total_fund_in'],
                        // totalFundIn : v['total_transaction'],
    ...
    function tableBtnClick(btnId) {
        ...
        if (btnName == 'view') {
            ...
            $.redirect("editMerchantDetails.php",{
