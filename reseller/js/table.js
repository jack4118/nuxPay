...
function buildTable(data, tableId, divId, thArray, btnArray, message, tableNo) {
    ...
    $.each(btnArray, function(key, val) {
        ...
        if (key == 'edit')
            ...
            iconArray.push('eye');
        else if (key == 'viewHide')
            iconArray.push('eye');
        else if (key == 'viewHidden')
            iconArray.push('eye');
        else if (key == 'cancel')
            ...
            iconArray.push('list-ul');
        else if (key == 'disable')
            iconArray.push('eye-slash');
        ...
        if(btnArray.length > 0 || !jQuery.isEmptyObject(btnArray)) {
            ...
            $.each(btnArray, function(key, value) {
                ...
                if(key=='viewHidden'){
                    ...
                    $('#'+tableId).find('tr#'+j+' td').last().append('<a data-toggle="tooltip" title="'+tooltipTitle+'" id="'+key+j+'" onclick="tableBtnClick(this.id)" class="m-t-5 m-r-10 btn btn-icon waves-effect waves-light btn-primary" style="background-color: #ff0000 !important; border: 1px solid #ff0000 !important;"><i class="fa fa-'+iconArray[k]+'"></i></a>');
                } else {
                    $('#'+tableId).find('tr#'+j+' td').last().append('<a data-toggle="tooltip" title="'+tooltipTitle+'" id="'+key+j+'" onclick="tableBtnClick(this.id)" class="m-t-5 m-r-10 btn btn-icon waves-effect waves-light btn-primary"><i class="fa fa-'+iconArray[k]+'"></i></a>');
                }
...
function buildTable_data(data, tableId, divId, thArray, btnArray, message, tableNo, tdAttr) {
    var btnArrayIsObject = jQuery.isPlainObject(btnArray) ? 1 : 0;
    ...
    while(j < data.length) {
        ...
        for(var i=0; i<col.length; i++) {
            if(tdAttrIsObject && tdAttr[i]){
                ...
                $('#'+tableId).find('tr#'+j).append('<td '+tdAttr[i]+'>'+nl2br(data[j][col[i]])+'</td>');
            }
            else {
                $('#'+tableId).find('tr#'+j).append('<td>'+data[j][col[i]]+'</td>');
            }
        ...
        if(btnArray.length > 0 || !jQuery.isEmptyObject(btnArray)) {
            ...
            $.each(btnArray, function(key, value) {
                ...
                if(key=='viewHidden'){
                    ...
                    if(data[j][col[13]]=="pending" && data[j][col[9]]=="sell" && data[j][col[10]] == "Auto Sell") {
                        $('#'+tableId).find('tr#'+j+' td').last().append('<a data-toggle="tooltip" title="'+tooltipTitle+'" id="'+key+j+'" onclick="tableBtnClick(this.id)" class="m-t-5 m-r-10 btn btn-icon waves-effect waves-light btn-primary"><i class="fa fa-'+iconArray[k]+'"></i></a>');
                    } else {
                        // $('#'+tableId).find('tr#'+j+' td').last().append('<a data-toggle="tooltip" title="'+tooltipTitle+'" id="'+key+j+'" onclick="tableBtnClick(this.id)" class="m-t-5 m-r-10 btn btn-icon waves-effect waves-light btn-primary"><i class="fa fa-'+iconArray[k]+'"></i></a>');
                    }
...
function pagination(pagerId, pageNumber, totalPage, totalRecord, numRecord) {
    ...
    var paginateMsg = "Showing %%from%% - %%to%% of %%total%% records."; 
